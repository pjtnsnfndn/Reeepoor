...
import requests
import re
import time  # added for measuring processing time
import jwt
...
# =====================================================================
# Global Dictionaries for Data Storage and Product ID Mapping
# =====================================================================
BUTTON_ALIASES = {}
...
# ----------------------- Product Short ID Mapping -----------------------
# برای جلوگیری از خطای callback_data طولانی، یک نگاشت بین نام محصول و شناسه کوتاه تعریف می‌کنیم.
PRODUCT_SHORT = {}
SHORT_PRODUCT = {}
...
def get_short_id(product: str) -> str:
    ...
    sid = "p" + str(abs(hash(product)) % 10000)
    PRODUCT_SHORT[product] = sid
    SHORT_PRODUCT[sid] = product
    return sid
...
# NEW: State for verifying buyer information from discount code in admin stats
VERIFY_BUYER_INPUT = 1200
# =====================================================================
# NEW: States for Checker Feature in Admin Panel
# =====================================================================
ADMIN_CHECKER_CHOICE = 9000
ADMIN_CHECKER_EASY_LINK_INPUT = 9001
# NEW: State for اکالا Checker input
ADMIN_CHECKER_EKALA_INPUT = 9002
...
def get_main_menu_keyboard():
    keyboard = [
        ...
        [KeyboardButton("📚راهنما")]  # New guide button added here
    ]
...
def get_admin_panel_keyboard():
    keyboard = [
        ...
        # NEW: Button for Checker Feature
        [InlineKeyboardButton("چکر‌ها 💻", callback_data="admin_checker")]
    ]
...
def get_product_purchase_keyboard():
    ...
    for product in PRODUCT_PRICES.keys():
        sid = get_short_id(product)
        buttons.append([InlineKeyboardButton(product, callback_data=f"buy_{sid}")])
...
# NEW: Guide Handler for the New "📚راهنما" Button
async def guide_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    guide_keyboard = InlineKeyboardMarkup([
        [InlineKeyboardButton("📌برنامه ی ایزی لینک", url="https://t.me/food_center_Channel/51")],
        [InlineKeyboardButton("⚠️رفع ارور های مربوط به ایزی", url="https://t.me/food_center_Channel/53")],
        [InlineKeyboardButton("🟢آموزش ورود به خط ایزی لینک", url="https://t.me/food_center_Channel/52")]
    ])
    await update.message.reply_text("راهنمای ایزی:", reply_markup=guide_keyboard)
...
# =====================================================================
# User Handlers
def get_charge_rules_keyboard():
    # دکمه‌های قوانین را می‌پذیرم و قوانین را رد میکنم
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("قوانین را می‌پذیرم ✅", callback_data="rules_accept")],
        [InlineKeyboardButton("قوانین را رد میکنم ❌", callback_data="rules_decline")]
    ])

async def show_charge_rules(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user = update.effective_user
    # زمان ارسال قوانین را ذخیره می‌کنیم
    RULES_TIMESTAMP[user.id] = time.time()
    rules_text = (
        "*📜قوانین استفاده از ربات فود سنتر 👇*\n\n"
        "📌مبالغ واریزی فقط برای خرید محصولات ربات هستند✅\n\n"
        "📌موجودی ربات قابل برگشت یا انتقال نیست ✅\n\n"
        "📌قبل از واریز و خرید از تصمیم خود اطمینان حاصل کنید زیرا محصولات پس گرفته نمی‌شود و پول هم برگشت داده نمی‌شود✅\n\n"
        "📌بی ادبی = بن دائم از ربات ✅\n\n"
        "📌سو استفاده ، فیش جعلی ، هویت جعلی و.... = بن از ربات ✅\n\n"
        "👤برای رفع مشکلات خود به پشتیبانی ربات مراجعه کنید🙂\n\n"
        "*با تشکر ، فود سنتر 🌹*"
    )
    # ارسال پیام قوانین به همراه کلیدهای انتخاب
    if update.message:
        await update.message.reply_text(rules_text, reply_markup=get_charge_rules_keyboard(), parse_mode="Markdown")
    elif update.callback_query:
        await update.callback_query.edit_message_text(rules_text, reply_markup=get_charge_rules_keyboard(), parse_mode="Markdown")

# ---------------------------------------------------------
# تغییر در هندلر شارژ حساب (charge_account)
# ---------------------------------------------------------
async def charge_account(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    user = update.effective_user
    # اگر کاربر قبلاً قوانین را تایید کرده باشد، به روند شارژ ادامه می‌دهیم
    if user.id in RULES_ACCEPTED:
        await update.message.reply_text("‍لطفاً روش پرداخت خود را انتخاب کنید :", reply_markup=get_payment_method_keyboard())
    else:
        # ارسال قوانین برای تایید
        await show_charge_rules(update, context)

# ---------------------------------------------------------
# هندلرهای مربوط به دکمه‌های قوانین
# ---------------------------------------------------------
async def rules_accept_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    await query.answer()
    user = query.from_user
    current_time = time.time()
    # بررسی اینکه حداقل 6 ثانیه از ارسال پیام قوانین گذشته باشد
    if user.id not in RULES_TIMESTAMP or (current_time - RULES_TIMESTAMP[user.id]) < 6:
        await query.edit_message_text("❌کاربر گرامی شما هنوز قوانین را مطالعه نکردید!")
        return
    # اگر پاسخ معتبر است، وضعیت تایید را ثبت می‌کنیم
    RULES_ACCEPTED.add(user.id)
    text = f"کاربر *{user.first_name}* شما قوانین را تایید کردید 🙂 هم‌اکنون می‌توانید حساب خود را شارژ کنید 🤩"
    await query.edit_message_text(text, parse_mode="Markdown")
    # اکنون می‌توانیم روند شارژ حساب را ادامه دهیم؛ برای مثال ارسال کیبورد روش‌های پرداخت
    await context.bot.send_message(chat_id=user.id, text="لطفاً روش پرداخت خود را انتخاب کنید :", reply_markup=get_payment_method_keyboard())

async def rules_decline_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    await query.answer()
    user = query.from_user
    text = f"کاربر *{user.first_name}* برای شارژ حساب باید قوانین را تایید کنید !"
    await query.edit_message_text(text, parse_mode="Markdown")
...
# در تابع buy_callback تغییر داده می‌شود:
async def buy_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    query = update.callback_query
    await query.answer()
    sid = query.data.split("_", 1)[1] if "_" in query.data else ""
    product = SHORT_PRODUCT.get(sid)
    if not product:
    ...
    # تغییر: ذخیره مبلغ خرید نیز همراه با تاریخ و نام محصول
    USER_RECENT_PURCHASES.setdefault(user_id, []).append((now, product, price))
...
async def buyer_verification_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    await query.edit_message_text("لطفاً کد تخفیف خود را ارسال کنید :")
    return VERIFY_BUYER_INPUT

async def buyer_verification_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    code_input = update.message.text.strip()
    found = False
    for user_id, purchases in USER_RECENT_PURCHASES.items():
        for record in purchases:
            # Expecting record structure: (timestamp, product, price, discount_code)
            if len(record) >= 4 and record[3] == code_input:
                now, product, price, discount_code = record
                username = USER_INFO.get(user_id, "نامشخص")
                reply = f"🔎کاربر *{username}* با آیدی عددی `{user_id}` و مبلغ *{price}* کد تخفیف `{discount_code}` را خریداری کرده✅"
        ...
        if found:
            break
    if not found:
        await update.message.reply_text("کد تخفیف وجود ندارد ❌")
    return ConversationHandler.END
# =====================================================================
...
# =====================================================================
# NEW: Checker Feature Implementation for Admin Panel
# =====================================================================

# The check_code function as provided for EasyLink checker
def check_code(code):
    url = f"https://api.easylink.sbs/search_by_code?code={code}"
    headers = {
...
# NEW: New function for checking اکالا discount using provided code snippet 
def check_ekala_code(mobile, password):
    ...
    payload = {
        ...
        "device_type_code": "7",
        "loginDuration": "267071"
    }
    headers = {"Content-Type": "application/x-www-form-urlencoded"}
    try:
        res = requests.post(login_url, data=payload, headers=headers, timeout=10)
    except:
        return False
    ...
    try:
        ...
        user_id = payload_decoded.get("userId") or payload_decoded.get("sub")
    except:
        return False
    if not user_id:
        return False
    discount_url = f"https://apigateway.okala.com/api/discount/v1/discounts/customer/{user_id}"
    headers["Authorization"] = f"Bearer {token}"
    headers["Accept"] = "application/json"
    try:
        res = requests.get(discount_url, headers=headers, timeout=10)
    except:
        return False
    return bool(res.status_code == 200 and res.json().get("data"))

# Handler for Checker menu in admin panel
async def admin_checker_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    await query.answer()
    keyboard = [
        [InlineKeyboardButton("🟣 چکر ایزی لینک ♻️", callback_data="checker_easylink")],
        [InlineKeyboardButton("🛒چکر اکالا", callback_data="checker_ekala")],
        [InlineKeyboardButton("منوی اصلی 🏠", callback_data="menu_main")]
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.edit_message_text("لطفا برنامه ی خود را انتخاب کنید :", reply_markup=reply_markup)

# Handler for selecting EasyLink checker
async def admin_checker_easylink_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    await query.edit_message_text("📌لطفاً خط‌های مربوط به کدها را ارسال کنید :", reply_markup=get_admin_cancel_keyboard())
    return ADMIN_CHECKER_EASY_LINK_INPUT

# NEW: Handler for selecting اکالا checker
async def admin_checker_ekala_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    message = ("📌لطفاً خط و رمز را به فرمت پایین ارسال کنید :\n\n"
               "➖مثال : 09223698742:123467fj")
    await query.edit_message_text(message, reply_markup=get_admin_cancel_keyboard())
    return ADMIN_CHECKER_EKALA_INPUT

# Handler for processing the provided codes for EasyLink checking
async def process_easylink_codes_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    text = update.message.text.strip()
    # Each line is a code; split by newline
    ...
    # Process each code with a 5-second delay between checks
    for code in codes:
        status = await asyncio.to_thread(check_code, code)
        if status == "Live":
            live_count += 1
        elif status == "Dead":
            dead_count += 1
    ...
    report = (
        "♛ Cʜᴇᴄᴋ sᴛᴀᴛᴜs\n"
        "➖➖➖➖➖➖➖➖➖➖\n"
        f"ϟ Aʟʟ ᴄᴏᴅᴇ : {total_codes}\n"
        "➖➖➖➖➖➖➖➖➖➖\n"
        f"✓ ʟɪᴠᴇ ᴄᴏᴅᴇ : {live_count}\n"
        "➖➖➖➖➖➖➖➖➖➖\n"
        f"☠ ᴅᴇᴀᴅ ᴄᴏᴅᴇ : {dead_count}\n"
        "➖➖➖➖➖➖➖➖➖➖\n"
        f"⏰ Tɪᴍᴇ sᴘᴇɴᴛ : {time_spent} seconds"
    )
    await update.message.reply_text(report)
    if dead_codes:
        dead_report = "❌ dead code:\n\n" + "\n".join(dead_codes)
        await update.message.reply_text(dead_report)
    return ConversationHandler.END

# NEW: Handler for processing اکالا codes using provided format
async def process_ekala_codes_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    text = update.message.text.strip()
    ...
    for line in lines:
        ...
        status = await asyncio.to_thread(check_ekala_code, mobile, password)
        if status:
            live_count += 1
        else:
            dead_count += 1
            dead_codes.append(line)
        await asyncio.sleep(5)
    time_spent = int(time.time() - start_time)
    report = (
        "♛ Cʜᴇᴄᴋ sᴛᴀᴛᴜs\n"
...
# =====================================================================
# NEW: Admin Handler for Forwarding a Global Message
# =====================================================================
async def admin_forward_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    query = update.callback_query
    await query.answer()
    await query.edit_message_text("لطفاً پیام همگانی که می‌خواهید فوروارد شود را ارسال کنید:")
...
async def charge_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if data == "charge_custom":
        message = (
            "💳 مبلغی را که می‌خواهید شارژ کنید (هرچقدر مد نظرتونه) برای شماره کارت زیر واریز کنید و رسید را به پیوی [[...]\n"
...
# ===================== Product Callback Handling with Short IDs =====================
# در این بخش، از شناسه کوتاه برای محصولات استفاده می‌کنیم.

# For buying product: در buy_callback از دیکشنری SHORT_PRODUCT استفاده می‌شود (همانطور که در بالا اعمال شد)

# ----------------- Admin Button Management Handlers -----------------
...
async def admin_remove_button_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for product in PRODUCT_PRICES.keys():
        sid = get_short_id(product)
        keyboard.append([InlineKeyboardButton(product, callback_data=f"remove_{sid}")])
...
async def admin_increase_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for product in PRODUCT_PRICES.keys():
        sid = get_short_id(product)
        keyboard.append([InlineKeyboardButton(product, callback_data=f"increase_{sid}")])
    keyboard.append([InlineKeyboardButton("منوی اصلی 🏠", callback_data="menu_main")])
...
async def admin_decrease_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for product in PRODUCT_PRICES.keys():
        sid = get_short_id(product)
        keyboard.append([InlineKeyboardButton(product, callback_data=f"decrease_{sid}")])
    keyboard.append([InlineKeyboardButton("منوی اصلی 🏠", callback_data="menu_main")])
...
async def admin_delete_code_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for product in SERVICE_CODES.keys():
        sid = get_short_id(product)
        keyboard.append([InlineKeyboardButton(product, callback_data=f"delete_{sid}")])
    keyboard.append([InlineKeyboardButton("منوی اصلی 🏠", callback_data="menu_main")])
...
# تغییر تابع برای ارسال پیام همگانی با پیشوند #پیام_همگانی
async def admin_broadcast_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    msg = update.message.text.strip()
    # افزودن پیشوند مورد نظر به متن پیام
    broadcast_msg = f"#پیام_همگانی\n\n{msg}"
    sent_count = 0
    for user in REGISTERED_USERS:
        try:
            await context.bot.send_message(chat_id=user, text=broadcast_msg)
            sent_count += 1
        except Exception as e:
            logger.error(f"خطا در ارسال پیام به کاربر {user}: {e}")
    # نمایش پیام نتیجه با تعداد موفقیت ارسال پیام به کاربرها
    await update.message.reply_text(
        f"📩پیام شما با موفقیت برای {sent_count} نفر ارسال - فوروارد شد✅",
        reply_markup=get_admin_panel_keyboard()
    )
    return ConversationHandler.END

# تغییر تابع برای فوروارد پیام همگانی جهت نمایش نتیجه با تعداد کاربرانی که پیام برایشان فوروارد شد
async def admin_forward_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await update.message.reply_text(
        f"📩پیام شما با موفقیت برای {forwarded_count} نفر ارسال - فوروارد شد✅",
        reply_markup=get_admin_panel_keyboard()
    )
    return ConversationHandler.END
...
def get_stats_panel_keyboard():
    keyboard = [
        ...
        [InlineKeyboardButton("🔎بررسی خریدار", callback_data="verify_buyer")]
    ]
    return InlineKeyboardMarkup(keyboard)
...
async def stats_products_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    for product in PRODUCT_PRICES.keys():
        sid = get_short_id(product)
        keyboard.append([InlineKeyboardButton(product, callback_data=f"stats_product_{sid}")])
    keyboard.append([InlineKeyboardButton("منوی اصلی 🏠", callback_data="menu_main")])
...
async def stats_product_details_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    await query.answer()
    sid = query.data.replace("stats_product_", "", 1)
    product = SHORT_PRODUCT.get(sid)
    if not product:
        await query.edit_message_text("محصول یافت نشد.", reply_markup=get_inline_main_menu())
        return
    price = PRODUCT_PRICES.get(product, 0)
    
    # Retrieve file paths related to the product.
    # If SERVICE_FILE_PATH[product] is not already a list, convert it to a list.
    paths = SERVICE_FILE_PATH.get(product, [])
    if not isinstance(paths, list):
        paths = [paths] if paths else []
    
    total_codes = 0
    # Sum the total codes from every file path associated with the product.
    for path in paths:
        if path and os.path.exists(path):
            try:
                ...
                logger.error(f"Error reading file {path} for {product}: {e}")
    
    # Calculate sold codes as the sum of all sold codes (from both previous and new file paths)
    sold_count = 0
    for purchases in USER_RECENT_PURCHASES.values():
        sold_count += sum(1 for (_, prod, *_) in purchases if prod == product)
    ...
    inline_kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("📌آمار فروش", callback_data=f"sales_stats_{sid}")],
        [InlineKeyboardButton("منوی اصلی 🏠", callback_data="menu_main")]
    ])
    await query.edit_message_text(message_text, reply_markup=inline_kb, parse_mode="Markdown")

# در تابع sales_stats_handler تغییر داده می‌شود:
async def sales_stats_handler(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    await query.answer()
    sid = query.data.replace("sales_stats_", "", 1)
    product = SHORT_PRODUCT.get(sid)
    if not product:
        await query.edit_message_text("محصول یافت نشد.", reply_markup=get_inline_main_menu())
        return
    # محاسبه مجموع مبلغ‌های واقعی خرید برای محصول مورد نظر از کدهای قبلی و جدید
    total_revenue = 0
    buyers = {}
    for user_id, purchases in USER_RECENT_PURCHASES.items():
        # فرض می‌کنیم هر خرید در قالب (timestamp, product, price_paid) ثبت شده است
        user_total = sum(price_paid for (_, prod, price_paid) in purchases if prod == product)
        if user_total:
            buyers[user_id] = user_total
            total_revenue += user_total
    message_text = f"💳درآمد کل : {total_revenue}\n"
    message_text += "👤کاربرانی که خریدند :\n"
    for user_id, amount in buyers.items():
        try:
            chat = await context.bot.get_chat(user_id)
            name = chat.first_name if chat.first_name else "نامشخص"
        except Exception:
            name = "نامشخص"
        message_text += f"👤کاربر ({name}) - `{user_id}` - *{amount}*\n"
    inline_kb = InlineKeyboardMarkup([
        [InlineKeyboardButton("بازگشت به آمار محصول", callback_data=f"stats_product_{sid}")],
        [InlineKeyboardButton("منوی اصلی 🏠", callback_data="menu_main")]
    ])
    await query.edit_message_text(message_text, reply_markup=inline_kb, parse_mode="Markdown")
...
async def admin_rename_button_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    for product in PRODUCT_PRICES.keys():
         sid = get_short_id(product)
         keyboard.append([InlineKeyboardButton(product, callback_data=f"rename_{sid}")])
...
async def admin_rename_button_select(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    sid = query.data.split("_", 1)[1]
    product = SHORT_PRODUCT.get(sid)
    context.user_data["original_button_name"] = product
...
async def admin_rename_button_input(update: Update, context: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not original_name or original_name not in PRODUCT_PRICES:
        ...
        return ConversationHandler.END
    # به جای تغییر کلیدهای دیکشنری‌های اصلی، یک نگاشت alias ثبت می‌کنیم.
    BUTTON_ALIASES[original_name] = new_name
...
async def profile_charge_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    message = (
        ...
        "🌐*توجه داشته باشید هیچ فردی به شماره‌ی شما دسترسی ندارد و شماره پیش ما محفوظ می‌ماند!*"
    )
...
if __name__ == '__main__':
    async def main():
        ...
        application = Application.builder().token("7612424903:AAEEhw1ihIF36-8oI1mefNUzwALj4BtfWaY").build()
        ...
        # New handler for the new "📚راهنما" button
        application.add_handler(MessageHandler(filters.Regex("^📚راهنما$"), guide_handler))
...
buyer_verification_conv = ConversationHandler(
    entry_points=[CallbackQueryHandler(buyer_verification_start, pattern="^verify_buyer$")],
    states={
        VERIFY_BUYER_INPUT: [MessageHandler(filters.TEXT & ~filters.COMMAND, buyer_verification_input)]
    },
    fallbacks=[
        CallbackQueryHandler(admin_cancel_handler, pattern="^admin_cancel$"),
        CommandHandler("cancel", admin_cancel_handler)
        ...
        application.add_handler(buyer_verification_conv)
        ...
        application.add_handler(CallbackQueryHandler(rules_accept_callback, pattern="^rules_accept$"))
        application.add_handler(CallbackQueryHandler(rules_decline_callback, pattern="^rules_decline$"))
        application.add_handler(CallbackQueryHandler(admin_banned_list_handler, pattern="^admin_banned_list_"))
        # NEW: Handler for Checker Feature
        application.add_handler(CallbackQueryHandler(admin_checker_handler, pattern="^admin_checker$"))
        application.add_handler(CallbackQueryHandler(admin_checker_easylink_handler, pattern="^checker_easylink$"))
        
        # Conversation for EasyLink Checker
        easylink_conv = ConversationHandler(
            entry_points=[CallbackQueryHandler(admin_checker_easylink_handler, pattern="^checker_easylink$")],
            states={
                ADMIN_CHECKER_EASY_LINK_INPUT: [MessageHandler(filters.TEXT & ~filters.COMMAND, process_easylink_codes_handler)]
            },
            fallbacks=[
                CallbackQueryHandler(admin_cancel_handler, pattern="^admin_cancel$"),
                CommandHandler("cancel", admin_cancel_handler)
        ...
        application.add_handler(easylink_conv)
        
        # ===================== NEW FEATURE: اکالا Checker =====================
        ekala_conv = ConversationHandler(
            entry_points=[CallbackQueryHandler(admin_checker_ekala_handler, pattern="^checker_ekala$")],
            states={
                ADMIN_CHECKER_EKALA_INPUT: [MessageHandler(filters.TEXT & ~filters.COMMAND, process_ekala_codes_handler)]
            },
            fallbacks=[
                CallbackQueryHandler(admin_cancel_handler, pattern="^admin_cancel$"),
                CommandHandler("cancel", admin_cancel_handler)
        ...
        application.add_handler(ekala_conv)
