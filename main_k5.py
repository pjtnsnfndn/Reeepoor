...
api_hash = '6d18eb5e69cfac7dfda54ce0fef6b07b'  # main api hash
bot_token = '7929810959:AAFbc736ro5MrBljqKFxUDpd3Soq5cNa_zM'  # main bot token
...
# Global variables for credit operations
tempAddCreditAmount = ""
tempAddCreditUser = ""
tempSubCreditAmount = ""
#           ---         ---         ---         #
...
def get_main_menu_keyboard():
    return InlineKeyboardMarkup([
        ...
        [InlineKeyboardButton('➕افزودن اعتبار به کاربر', callback_data='addCredit'),
         InlineKeyboardButton('➖کم‌ کردن اعتبار کاربر', callback_data='subCredit')]
    ])
...
async def callbackQueries(client, query):
    ...
    if chat_id in bot_admins:
        ...
        # Updated group report submenu: each button on its own line
            ...
            await bot.edit_message_text(chat_id, message_id, '<b>لطفاً ابتدا لینک پیام مورد نظر را ارسال کنید :</b>')
        # Newly added callbacks for credit operations
        elif data == 'addCredit':
            step = 'addCreditAmount'
            await bot.edit_message_text(chat_id, message_id, '<b>مقدار اعتبار را جهت افزودن وارد کنید:</b>')
        elif data == 'subCredit':
            step = 'subCreditAmount'
            await bot.edit_message_text(chat_id, message_id, '<b>مقدار اعتبار را جهت کسر وارد کنید:</b>')
        # Existing callbacks for account functions
        elif data == 'accountsList':
            accounts = await accountList()
            if accounts:
                await bot.edit_message_text(chat_id, message_id, '<b>لیست اکانت‌ها:</b>\n' + '\n'.join(accounts))
            else:
                await bot.edit_message_text(chat_id, message_id, '<b>هیچ اکانتی موجود نیست.</b>')
        elif data == 'checkAccounts':
            accounts = await accountList()
            if accounts:
                await bot.edit_message_text(chat_id, message_id, '<b>تعداد اکانت‌های موجود: </b>' + str(len(accounts)))
            else:
                await bot.edit_message_text(chat_id, message_id, '<b>هیچ اکانتی یافت نشد.</b>')
...
async def TextResponse(client, message):
    ...
    global tempAddCreditAmount, tempAddCreditUser, tempSubCreditAmount
...
• زمان سپری شده : {spendTime}''')
    ...
    # For adding credit
    elif step == 'addCreditAmount':
        tempAddCreditAmount = text.strip()
        step = 'addCreditUser'
        await message.reply('<b>آیدی عددی کاربر را اضافه کنید:</b>')
    elif step == 'addCreditUser':
        tempAddCreditUser = text.strip()
        await message.reply('<b>اعتبار اضافه شد ✅</b>')
        await message.reply(f'<b>کاربر {tempAddCreditUser} مبلغ {tempAddCreditAmount} به اعتبار شما اضافه شد✅🌹</b>')
        step = None
    # For subtracting credit
    elif step == 'subCreditAmount':
        tempSubCreditAmount = text.strip()
        await message.reply('<b>کسر شد ✅</b>')
        userName = message.from_user.first_name if message.from_user.first_name else "کاربر"
        await message.reply(f'<b>کاربر {userName} مبلغ {tempSubCreditAmount} از اعتبار شما کسر شد ❌</b>')
        step = None
